...
namespace BanjoBot
{
    class Program
    {
        ...
        private static List<MyServer> _allServers;
        ...
        private static void LoadServerInformation(Server server)
        {
            ...
            MyServer myServer = new MyServer(server);  //TODO: createServerFromSql, createLeagueFromSql, createPlayersfromSql
            if (myServer != null)
            {
                _allServers.Add(myServer);
                _ls = new LeagueController(myServer, null); //TODO: Move to MyServer Constructor _
            }
            else
            {
        ...
        {
            foreach (MyServer myServer in _allServers)
            {
        ...
        private MyServer getMyServer(Server server)
        {
            foreach (MyServer myServer in _allServers) {
                if (myServer.DiscordServer.Id == server.Id)
                    return myServer;
