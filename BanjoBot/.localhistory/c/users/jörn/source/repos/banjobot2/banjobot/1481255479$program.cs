...
namespace BanjoBot
{
    class Program
    {
        private const String BBL_SIGNUP_URL = "www.goo.gl/JbOfGE";
        ...
        {
            e.Channel.SendMessage(e.Exception.Message);
        }
        ...
        private static bool CheckIfRegistered(Command command, User user, Channel channel)
        {
            foreach(League league in _leagues)
            {
                ...
                {
                    Console.WriteLine("RegisterCheck success");
                    return true;
                }
                   
            }
            Console.WriteLine("RegisterCheck failed");
            return false;
        }
